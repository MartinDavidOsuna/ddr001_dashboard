...
describe('Construction real data UI', () => {
  ...
  it('only offers assignable roles and distinguishes a successful write from failed refresh', async () => {
    ...
    mock.access.mockResolvedValueOnce({ rowVersion: '0x0000000000000001', userId: 'u1', role: 'contractor', accessEnabled: true, ownSurveyCount: 1 })
    const wrapper = mount(Access, { props: { userId: 'u1' } })
    await flushPromises()
    ...
    await wrapper.get('input[maxlength="500"]').setValue('Cambio autorizado de prueba')
    await wrapper.get('button.save').trigger('click')
    await flushPromises()
    expect(mock.save).toHaveBeenCalledWith('u1', 'resident', 'Cambio autorizado de prueba', '0x0000000000000001')
